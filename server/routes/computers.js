...
router.get('/with-bookings', verifyToken, async (req, res) => {
  try {
    ...
    const computersWithBookings = await Promise.all(computers.map(async (computer) => {
      ...
      if (hasActiveBookings && computer.status !== 'maintenance') {
        ...
        if (computer.status !== 'booked') {
          await Computer.findByIdAndUpdate(computer._id, { status: 'booked' });
        }
      } else if (!hasActiveBookings && computer.status === 'booked' && computer.status !== 'maintenance') {
        updatedStatus = 'available';
        // Update the computer status in database if it should be available
        await Computer.findByIdAndUpdate(computer._id, { status: 'available' });
      }
      ...
      return {
        ...
        nextAvailable: bookings.length > 0 ? 
          bookings[bookings.length - 1].endTime : null,
        nextAvailableDate: bookings.length > 0 ? 
          bookings[bookings.length - 1].date : null
      };
    }));
    ...
    res.status(500).json({ message: 'Error fetching computers with booking information' });
  }
});
